...
let Player = {
    ...
    tooglePlayer:(accion = 'open') => {
        ...
        if(accion == "open"){
            ...
            TweenMax.to("#btn_abrir_player",0.1,{bottom:-52});
...
//            console.log(err.message);
    ...
    ponVideo:(i) =>{
        ...
        var $video = $(`<video no-controls muted playsinline id="video_${i}" width="450px" height="560px"/>`);
//        var $video = $(`<video no-controls id="video_${i}" width="450px" height="560px"/>`);
    ...
    ponVideoInteraccion:(i) =>{
        ...
        var $video = $(`<video no-controls muted playsinline id="video_interaccion_${i}" width="450px" height="560px"/>`);
        var $source = $('<source type="video/mp4"/>');
        var src = videoTarget;
        var blob = new Blob( [ src ], { type: "video/mp4" } );
        var urlCreator = window.URL || window.webkitURL;
        ...
        $source.attr('src', objUrl);
        $video.append($source);
        $('.video_container').append($video); // <---- append en vez de sobreescribir el div
        ...
        try{vid.addEventListener("ended", Player.hideSenasOnEnd);} catch (err) {console.log('no se puede agregar listener', err.message);}
    },
...
//                console.log(posicion, delay, `${i}_${j}`, desfase, delayAcumulado, tiempo);
        ...
        _arr.map((item, i) => {
            ANIM.main_tl.addCallback(function(){
                 Player.playVideo(Player.getTimeVideoFromCurrentPosition());
            },`escena_${i}+=${Player.getDelayVideo(i)}`);
        });
    }
};
...
//                    $('#player_btn_config').css({'margin-left':'auto'}); 
    ...
    setSenasControllers:() => {
        ...
        if(Utils.isMobile()){
            ...
            btnFlecha.addEventListener('touchstart', function(event) {
                 Player.toogleBarraAlphaVideo('open');
            }, false);
            
            btnFlecha.addEventListener('touchend', function(event) {
                 Player.forceCloseBarraAlphaVideo();
            }, false);
            
            $("#senas_container .controllers_container .alpha_controller .flecha").off();
        } 
